...
class ComplexDiagonalDynamicOperator(AbstractDynamicOperator):
    def __init__(self, dim: int, num_operations: int):
        ...
        self.real = nn.Parameter(torch.Tensor(self.num_operations, self.dim // 2))
        nn.init.xavier_normal_(self.real.data)
        self.imag = nn.Parameter(torch.Tensor(self.num_operations, self.dim // 2))
        nn.init.xavier_normal_(self.imag.data)
