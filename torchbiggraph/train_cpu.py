...
from torchbiggraph.train_util import IterationManager,get_num_edge_chunks,Trainer
...
class TrainingCoordinator:
    ...
    def _swap_partitioned_embeddings(
        ...
        if new_b is not None:
            ...
            logger.info("Loading partitioned embeddings")
            for entity, part in new_parts - old_parts:
                if (entity, part) in self.embeddings_buffer:
                    logger.debug(f"Loading ({entity} {part}) from buffer")
                    embs,optimizer=self.embeddings_buffer[entity, part]
                    holder.partitioned_embeddings[entity, part] = embs
                    self.trainer.partitioned_optimizers[entity, part] = optimizer
                else:
                    if len(self.embeddings_buffer)>=self.buffer_size:
                        for entity_,part_ in self.embeddings_buffer:
                            if entity_==entity and (entity, part_) not in holder.partitioned_embeddings and (entity, part_) not in new_parts - old_parts:
                                break
                        embs,optimizer=self.embeddings_buffer.pop((entity_, part_))
                        self.checkpoint_manager.write(
                            entity, part_, embs.detach(), optimizer.state_dict()
                        )
                        self.embedding_storage_freelist[entity].add(embs.storage())
                        
                    logger.debug(f"Loading ({entity} {part}) from checkpoint")
                    ...
                    holder.partitioned_embeddings[entity, part] = embs
                    self.trainer.partitioned_optimizers[entity, part] = optimizer
                    self.embeddings_buffer[entity, part]=(embs,optimizer)
                    io_bytes += embs.numel() * embs.element_size()  # ignore optim state

        assert new_parts == holder.partitioned_embeddings.keys()
        #assert new_parts in holder.partitioned_embeddings.keys()
    ...
    def _eval(self):
        do_eval(self.eval_config,None,None,self.holder,self.subprocess_init)
...
#################################################################################################################
        version=self.checkpoint_manager._version(True)
        if preserve_new_checkpoint or (version == config.num_epochs):
            for entity,part in self.embeddings_buffer:
                embs,optimizer=self.embeddings_buffer[entity,part]
                self.checkpoint_manager.write(
                    entity, part, embs.detach(), optimizer.state_dict()
                )
